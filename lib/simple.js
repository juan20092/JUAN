...
export function smsg(conn, m) {
  ...
  m.download = async (filename) => {
    ...
    const stream = await conn.downloadMediaMessage(m.msg)
    if (filename) {
      const fs = await import('fs')
      await fs.promises.writeFile(filename, stream)
      return filename
    }
    return stream
  }
 ...
 * @returns {Promise<Buffer|string>} Buffer or filename
 */
export async function downloadMediaMessage(message, filename) {
  const stream = await this.downloadContentFromMessage(message, 'buffer')
